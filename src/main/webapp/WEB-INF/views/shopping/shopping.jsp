...
<c:if test="${sessionScope.Naver_name != null}">
	<title>${sessionScope.Naver_name}의장바구니</title>
</c:if>

<c:if test="${sessionScope.Google.name != null}">
	<title>${sessionScope.Google_name}의장바구니</title>
</c:if>
<!-- BootStrap -->
			...
			<c:forEach var="cart" items="${shopping}">
				...
				<table border="1">
					<tr>
						...
						<td style="font-family: 'BMJUA';">/
							<button type="button" id="delcart">메뉴 삭제</button>
						</td>
...
-->
				...
				<input type="hidden" value="${cart.s_num}">
				<input type="hidden" value="${cart.s_name}">

			</c:forEach>

			<input type="hidden" value="${total_price}" name="price"> <input
				type="hidden" value="${total_quan}" name="quan">
